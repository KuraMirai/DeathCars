Shader "Hidden/HologramShaderWithStencil (SoftMaskable)"
{
    Properties
    {
        // General
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                // Glitches
                #if _GLITCH_ON
                v.vertex.x += _GlitchIntensity * (step(0.5, sin(_Time.y * 2.0 + v.vertex.y * 1.0)) * step(
                    0.99, sin(_Time.y * _GlitchSpeed * 0.5)));
                #endif
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                col.a *= SoftMask(i.vertex, i.worldVertex);
                col.rgb *= _Brightness;
